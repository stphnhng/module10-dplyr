...
flights <- mutate(flights, time.gain = arr_delay - dep_delay) %>% arrange(-time.gain)
...
seatac.frame <- filter(flights, dest == "SEA") %>% select(origin,dest,time.gain)

# On average, did flights to seatac gain or loose time?

mean(seatac.frame$time.gain, na.rm = TRUE)

# Consider flights from JFK to SEA. What was the average, min, and max air time
# of those flights? Bonus: use pipes to answer this question in one statement!
# (and don't show any other data)

jfk.to.sea.time <- filter(flights, origin == "JFK", dest == "SEA") %>% select(air_time)
mean(jfk.to.sea.time$air_time, na.rm = TRUE)
min(jfk.to.sea.time$air_time, na.rm = TRUE)
max(jfk.to.sea.time$air_time, na.rm = TRUE)
